...
export async function fetchUSAJobs(opts?: { limit?: number; keyword?: string; remoteOnly?: boolean }): Promise<Job[]> {
  ...
  if (!userAgentEmail) {
    throw new Error('USAJOBS API not configured (missing USAJOBS_USER_AGENT_EMAIL).');
  }
